...
import jp.yoima.manaita.block.ManaitaHookBlock;
...
public final class ModBlocks {
    ...
    private static final Map<Item, ManaitaTier> BOARD_TIERS = new HashMap<>();
    ...
    public static final ManaitaHookBlock MANAITA_HOOK = registerSimpleBlock("manaita_hook",
            blockKey -> new ManaitaHookBlock(AbstractBlock.Settings.copy(Blocks.IRON_BARS).strength(0.375f).nonOpaque().sounds(BlockSoundGroup.METAL).registryKey(blockKey))
    );
    ...
    private static ManaitaBoardBlock registerBoard(ManaitaTier tier, AbstractBlock.Settings settings) {
        ...
        BOARD_TIERS.put(item, tier);
        return block;
    }

    private static <T extends Block> T registerSimpleBlock(String id, java.util.function.Function<RegistryKey<Block>, T> factory) {
        RegistryKey<Block> blockKey = RegistryKey.of(RegistryKeys.BLOCK, ManaitaMod.id(id));
        RegistryKey<Item> itemKey = RegistryKey.of(RegistryKeys.ITEM, ManaitaMod.id(id));
        T block = factory.apply(blockKey);
        T registeredBlock = Registry.register(Registries.BLOCK, blockKey, block);
    ...
    public static boolean isBoardItem(ItemStack stack) {
        return tierForBoard(stack) != null;
    }

    public static ManaitaTier tierForBoard(ItemStack stack) {
        if (stack.isEmpty()) {
            return null;
        }
        return BOARD_TIERS.get(stack.getItem());
    }
